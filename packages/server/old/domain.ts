...
export default class ServerDomain extends Domain {
   ...
   */
  override async load(settings: OnlyServerSettingsType) {
    await buildLogging();
  ...
  override async ready(options: DomainReadyFunctionArgs<OnlyServerSettingsType>) {
    /*const domainsAsServerDomains = (options.domains || []) as ServerDomainInterface[];

    if (options.app instanceof HttpAppServer) {
      await this.app.startRoutes(domainsAsServerDomains.filter((domain) => typeof domain.getRoutes === 'function'));
