 ...
 * @generics TResponses - The responses that the handler can return. This is used for documentation and also will exist on the request object so you can retrieve it at any time and throw passing the parameters you want.
 */
...
/**
 * Used for appending the handlers to the router. This way we can then extract all of the handlers of a given router. It is defined by each method it defines.
 *
 * @generics TRootPath - The path from the router, this is the full path by joining all the routers together.
 ...
 * This is used for knowing what type of {@link Request} is expected for the handler and what type of {@link Response} will it return.
 *
 * @generics TRouters - A list of {@link BaseRouter} or {@link Omit<BaseRouter, never>} that will be used for extracting the handlers.
 * @generics TFinalHandlers - The final type of the handlers, this is used for recursion and should not be used by the user.
 *
 ...
 *
 * @generics TIncludes - A tuple of routers that will be used for extracting the routers. We will traverse through the parent and all it's children.
