// Pretty much this hole file was copied from the next.js repo:
// https://github.com/vercel/next.js/blob/canary/packages/react-dev-overlay/src/internal/helpers/launchEditor.ts
// Give them proper credit, and let's try to keep it up to date.
// We modified it so it uses our own std lib with that, this piece of code will work
// on any environment without bundling.
...
async function getArgumentsForLineNumber(
  defaultStd: typeof std,
  editor: string,
  fileName: string,
  lineNumber: number,
  colNumber: number
): Promise<string[]> {
  const editorBasename = (await defaultStd.files.basename(editor)).replace(/\.(exe|cmd|bat)$/i, '');
...
async function guessEditor(
  defaultStd: typeof std,
  platform: Awaited<ReturnType<(typeof std)['files']['basename']>>
): Promise<string[]> {
  ...
  try {
    if (isMacOs) {
      ...
      const output = await defaultStd.childProcess.executeAndOutput(
      ...
      for (let i = 0; i < runningProcesses.length; i++) {
        ...
        if (COMMON_EDITORS_WIN.includes(processName)) return [processPath];
      }
...
async function printInstructions(defaultStd: typeof std, fileName: string, errorMessage: string | null) {
  const baseNameOfFileName = defaultStd.files.basename(fileName);
  console.log(`\nCould not open ${baseNameOfFileName} in the editor.\n`);
  if (errorMessage) {
...
async function launchEditor(fileName: string, lineNumber: number, colNumber: number) {
  const doesNotExistFile = (await std.files.exists(fileName)) === false;
  if (doesNotExistFile) return;
  ...
  const [osRelease, osPlatform] = await Promise.all([std.os.release(), std.os.platform()]);
  // eslint-disable-next-line prefer-const
  let [editor, ...args] = await guessEditor(std, osPlatform);

  if (!editor) return printInstructions(std, fileName, null);

  if (editor.toLowerCase() === 'none') return;

  if (osPlatform === 'linux' && fileName.startsWith('/mnt/') && /Microsoft/i.test(osRelease)) {
    ...
    fileName = await std.files.relative('', fileName);
  }
  ...
  if (osPlatform === 'windows' && !WINDOWS_FILE_NAME_ACCESS_LIST.test(fileName.trim())) {
    const baseNameOfFileName = await std.files.basename(fileName);
    console.log(`\nCould not open ${baseNameOfFileName} in the editor.\n`);
    console.log(
  ...
  const argumentsForLineNumber = await getArgumentsForLineNumber(std, editor, fileName, lineNumber, colNumber);
  if (lineNumber) args = args.concat(argumentsForLineNumber);
  else args.push(fileName);

  const onErrorOnProcess = (error: Error) => printInstructions(std, fileName, error.message);
  const onExitOnProcess = (errorCode: number | null) => {
    if (errorCode) printInstructions(std, fileName, '(code ' + errorCode + ')');
  };

  if (osPlatform === 'windows')
    // On Windows, launch the editor in a shell because spawn can only
    // launch .exe files.
    std.childProcess.spawn('cmd.exe', ['/C', editor].concat(args), {
      stdio: ['inherit'],
      detached: true,
      onError: onErrorOnProcess,
      onExit: onExitOnProcess
    });
  else if (isTerminalEditor(editor)) printInstructions(std, fileName, 'Terminal editors are not supported.');
  else
    std.childProcess.spawn(editor, args, {
      stdio: ['inherit'],
      onError: onErrorOnProcess,
      onExit: onExitOnProcess
    });
