...
function getArgumentsForLineNumber(editor: string, fileName: string, lineNumber: number, colNumber: number): string[] {
...
function printInstructions(fileName: string, errorMessage: string | null) {
  console.log();
  console.log(console.log('Could not open ' + path.basename(fileName) + ' in the editor.'));
  if (errorMessage) {
    if (errorMessage[errorMessage.length - 1] !== '.') {
      errorMessage += '.';
    }
    console.log(console.log('The editor process exited with an error: ' + errorMessage));
  }
  console.log();
  console.log(
    'To set up the editor integration, add something like ' +
      console.log('REACT_EDITOR=atom') +
      ' to the ' +
