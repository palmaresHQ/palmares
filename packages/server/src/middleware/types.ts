...
type Exact<TOne, TTwo> =
  (<T>() => T extends TOne ? 1 : 0) extends <T>() => T extends TTwo ? 1 : 0
    ? TOne extends TTwo
      ? TTwo extends TOne
        ? true
        : false
...
> = TMiddlewares extends readonly [infer TFirstMiddie, ...infer TRestMiddlewares]
  ? TFirstMiddie extends Middleware
    ? // On this use case the user might not have defined the request function, so we need to check if it's defined.
      // If we don't do this, it'll return never.
      ReturnType<
            ...
            ? Request<
                TPath,
                {
                  ...
                  responses: TFirstMiddie extends {
                    options: { responses: infer TResponses extends MiddlewareOptions['responses'] };
                  }
                ...
                ? Request<
                    TPath,
                    {
                      ...
                      responses: RemoveOptionals<Extract<TRequest, Request<any, any>>['responses']> &
                        (RemoveOptionals<TFinalRequest['responses']> extends undefined
                          ? unknown
                          : RemoveOptionals<TFinalRequest['responses']>) &
                        (RemoveOptionals<TResponses> extends undefined ? unknown : RemoveOptionals<TResponses>);
                      method: TMethod;
        ...
        ? TRequestServer extends Request<any, any>
          ? TPreviousServerRequest extends Request<any, any>
            ? Exact<
              ...
              > extends false // Prevents adding the requests modified by the middlewares, we should guarantee that the
              ? //request is explicitly defined by the user.
