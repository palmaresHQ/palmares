...
export default async function buildLogging() {
  const defaultLoggingForDatabases = (message: string) =>
    `\x1b[1m[databases]\x1b[0m ${message}`;
  logging.appendMessage(
    LOGGING_DATABASE_MODELS_NOT_FOUND,
    MessageCategories.Warn,
    async ({ domainName }) =>
      defaultLoggingForDatabases(
  ...
  logging.appendMessage(
    ...
    async ({ databaseName }) =>
      defaultLoggingForDatabases(
        `Closing the '${databaseName}' database connection.`
      )
  );
  logging.appendMessage(
    LOGGING_DATABASE_IS_NOT_CONNECTED,
    ...
    async ({ databaseName }) =>
      defaultLoggingForDatabases(
        `Couldn't connect to the '${databaseName}' database.`
      )
  );
  logging.appendMessage(
    LOGGING_MIGRATIONS_NOT_FOUND,
  ...
  logging.appendMessage(
    LOGGING_MIGRATIONS_FILE_DESCRIPTION,
    MessageCategories.Info,
    async ({ database, lastMigrationName, lastDomainPath }) =>
      defaultLoggingForDatabases(
  ...
  logging.appendMessage(
    ...
    MessageCategories.Info,
    async ({ title }) =>
      defaultLoggingForDatabases(`Running migration: \x1b[36m${title}`)
  );
  logging.appendMessage(
    LOGGING_MIGRATIONS_ACTION_DESCRIPTION,
    MessageCategories.Info,
  ...
  logging.appendMessage(
    ...
    MessageCategories.Info,
    async () =>
      defaultLoggingForDatabases(`No changes were found in your models.`)
  );
}
