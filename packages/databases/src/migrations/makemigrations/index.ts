...
import { FRAMEWORK_NAME, logging, Domain, imports } from '@palmares/core';
...
export default class MakeMigrations {
  ...
  async #generateMigrationFile(
    ...
    if (join && existsSync && mkdir && writeFile && readdir) {
      ...
      if (!existsSync(pathToWriteMigrations)) {
        ...
        await new Promise((resolve, reject) => {
          writeFile(
            join(pathToWriteMigrations, indexFileOfMigrations),
            '',
            (error) => {
      ...
      // This will write the migrations to index.ts or .js file.
      await new Promise((resolve, reject) => {
        readdir(pathToWriteMigrations, (error, files) => {
