 ...
 * implementation.
 * - `translate` - Used to translate the CharField to something that the database can understand.
 ...
 */
export function adapterCharFieldParser<
  TTranslateFunction extends AdapterCharFieldParser['translate'],
  TInputParserFunction extends NonNullable<AdapterCharFieldParser>['inputParser'],
  TOutputParserFunction extends NonNullable<AdapterCharFieldParser>['outputParser']
>(args: {
  /**
   * @description
   ...
   * to something that the database can understand. It's nice if all of the configuration options
   ...
   * `Date` objects for `CharField`. But your database does not support saving `Date` instances. What you can do is
   * that you can implement this method on `CharFieldParser` and return a iSO string. With that, you can be 100% sure
   ...
   * `CharField` from the database, you can parse the value to a `Date` object.
  ...
  return CustomAdapterCharFieldParser as typeof AdapterCharFieldParser & {
    new (): AdapterCharFieldParser & {
      translate: TTranslateFunction;
      inputParser: TInputParserFunction;
      outputParser: TOutputParserFunction;
    };
 ...
 */
export class AdapterCharFieldParser {
  /**
   * @description
   * Used to translate the field to something that the database can understand. The `{@link AdapterFieldParser}`
   * instance will be injected by default in the `translate` method.
   ...
   * This is used to parse the input value before you save it. For example, let's say that palmares by default accept
   * `Date` objects for `CharField`. But your database does not support saving `Date` instances. What you can do is
   * that you can implement this method on `CharFieldParser` and return a iSO string. With that, you can be 100%
   * sure that the data on your `queryData` is something valid for your database.
   *
   * This parses the value for each data.
   *
   * @example
  ...
  /**
   * @description
   * This is used to parse the output value before you send it to the user. For example, if the user is fetching
   * a `CharField` from the database, you can parse the value to a `Date` object.
   * This can be useful so you can guarantee that the user will receive the data in the format that it's expected.
   *
   * This parses the value for each data that is retrieved.
   *
