 ...
 *
 * This will be used to parse the fields that are going to be used in the model in the database, we call this class just for the `CharField`
...
>(args: {
  ...
  /**
   * @description
   * This is used to parse the input value before you save it. For example, let's say that palmares by default accept `Date` objects for `CharField`. But your database does not support saving
   * `Date` instances. What you can do is that you can implement this method on `CharFieldParser` and return a iSO string. With that, you can be 100% sure that the data on your `queryData` is
  ...
  /**
   * @description
   * This is used to parse the output value before you send it to the user. For example, if the user is fetching a `CharField` from the database, you can parse the value to a `Date` object.
 ...
 * in the `DatabaseAdapter` class constructor.
 */
export default class AdapterCharFieldParser {
  /**
   * @description
   * Used to translate the field to something that the database can understand. The `{@link AdapterFieldParser}` instance will be injected by default in the `translate` method.
   * The core idea is that for every field type we will have a parser that will be used to translate the field to something that the database can understand. It's nice if all of the configuration options
