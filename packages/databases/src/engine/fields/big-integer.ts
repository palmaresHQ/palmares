 ...
 *
 * This will be used to parse the fields that are going to be used in the model in the database, we call this class just for the `BigIntegerField`
 ...
 * - `translate` - Used to translate the BigIntegerField to something that the database can understand.
 ...
 */
export function adapterBigIntegerFieldParser<
  TTranslateFunction extends AdapterBigIntegerFieldParser['translate'],
  TInputParserFunction extends Required<AdapterBigIntegerFieldParser>['inputParser'],
  TOutputParserFunction extends Required<AdapterBigIntegerFieldParser>['outputParser'],
>(args: {
  /**
   * @description
  ...
  class CustomAdapterBigIntegerFieldParser extends AdapterBigIntegerFieldParser {
    translate = args.translate;
    inputParser = args.inputParser as TInputParserFunction;
    outputParser = args.outputParser as TOutputParserFunction;
  }

  return CustomAdapterBigIntegerFieldParser as typeof AdapterBigIntegerFieldParser & {
    new (): AdapterBigIntegerFieldParser & {
      translate: TTranslateFunction;
      inputParser: TInputParserFunction;
      outputParser: TOutputParserFunction;
    };
 ...
 * in the `DatabaseAdapter` class constructor.
 */
export default class AdapterBigIntegerFieldParser {
  /**
   * @description
   * Used to translate the field to something that the database can understand. The `{@link AdapterFieldParser}` instance will be injected by default in the `translate` method.
   * The core idea is that for every field type we will have a parser that will be used to translate the field to something that the database can understand. It's nice if all of the configuration options
  ...
  async translate(args: AdapterFieldParserTranslateArgs<'big-integer'>): Promise<any> {
    throw new EngineDoesNotSupportFieldTypeException(args.engine.constructor.name, BigIntegerField.name);
  }
   ...
   */
  // eslint-disable-next-line ts/require-await
  async inputParser?(args: AdapterFieldParserInputAndOutputArgs<'big-integer'>) {
    return args.value;
  }
   ...
   */
  // eslint-disable-next-line ts/require-await
  async outputParser?(args: AdapterFieldParserInputAndOutputArgs<'big-integer'>) {
    return args.value;
  }
}
