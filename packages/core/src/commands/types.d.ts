...
type ExtractTypeFromArg<TArgument extends NonNullable<NonNullable<Domain['commands']>[string]['keywordArgs']>[string] | NonNullable<NonNullable<Domain['commands']>[string]['positionalArgs']>[string], TPositionalOrKeyword extends 'positionalArgs' | 'keywordArgs'> = TArgument['type'] extends 'string' ? TArgument['canBeMultiple'] extends true ? string[] : string : TArgument extends 'number' ? TArgument['canBeMultiple'] extends true ? number[] : number : TArgument extends 'boolean' ? TArgument['canBeMultiple'] extends true ? boolean[] : boolean : TArgument extends string[] | readonly string[] ? TArgument['canBeMultiple'] extends true ? Narrow<TArgument> : Narrow<TArgument> : TPositionalOrKeyword extends 'positionalArgs' ? TArgument['canBeMultiple'] extends true ? string[] : string : TArgument['canBeMultiple'] extends true ? boolean[] : boolean;
/**
 * Used for extracting the type of the arguments of a command of a specific domain for a specific command.
 */
export type ExtractCommandsType<TDomain extends typeof Domain | ReturnType<typeof domain>, TCommand extends keyof NonNullable<InstanceType<TDomain>['commands']>> = {
    keywordArgs: {
        [Key in keyof NonNullable<InstanceType<TDomain>['commands']>[TCommand]['keywordArgs']]?: ExtractTypeFromArg<NonNullable<NonNullable<InstanceType<TDomain>['commands']>[TCommand]['keywordArgs']>[Key], 'keywordArgs'>;
    };
    positionalArgs: {
        [Key in keyof NonNullable<InstanceType<TDomain>['commands']>[TCommand]['positionalArgs']]: ExtractTypeFromArg<NonNullable<NonNullable<InstanceType<TDomain>['commands']>[TCommand]['positionalArgs']>[Key], 'positionalArgs'>;
    };
};
