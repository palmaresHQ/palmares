 ...
 * @param isSnake - If true, the string will be converted to snake_case, if false, it will be converted to hyphen-case.
 * Default to true.
 *
...
/**
 * Creates a deep copy of an object. If you do {...obj} we will create a new reference of the object but we will not
 * deeply clone the objects inside of that object. This means that changes for nested objects will be applied to all
 * objects. The same happens if we are trying to create a new copy of an array.
 *
 * @param objectOrArray - The object or array to be deeply copied.
 * @returns - The object copy.
 */
export function structuredClone<TObjectOrArray extends Record<string | number, any> | any[]>(
  objectOrArray: TObjectOrArray
): TObjectOrArray {
  ...
  if (Array.isArray(objectOrArray)) {
    ...
    for (let i = 0; i < objectOrArray.length; i++) {
      const valueAtIndex =
        !objectOrArray[i] || typeof objectOrArray[i] !== 'object'
          ? objectOrArray[i]
          : structuredClone(objectOrArray[i]);
  ...
  for (const key of Object.keys(objectOrArray)) {
    const valueAtKey =
      !objectOrArray[key] || typeof objectOrArray[key] !== 'object'
        ? objectOrArray[key]
        : structuredClone(objectOrArray[key]);
