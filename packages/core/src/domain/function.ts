...
/**
 * Functional approach to how one can create a domain. It's similar to the class approach,
 * but can be more flexible then the class counter-part.
 *
 * @example ```ts
...
>(
  ...
  name: string,
  /**
   * This is where the domain is located, this way we can write files directly to the
   * domain folder. On Node.js just use `__dirname`.
   */
  path: string,
  args: {
    /**
     * Modifier classes are NOT used for anything else besides to guarantee that you actually pass all
     * of the required arguments to the domain.
     */
    modifiers?: TModifiers;
    commands?: TCommands;
    /**
     * Function that will be called once the domain is loaded. It will receive the settings
     * as the first argument.
     * Take note that the settings object appends the settings for each domain it reads in
     * order, so the domain order is really important.
     */
    load?: TLoadFunction;
    /**
     * Function that will be called once the application is ready. Application ready means
     * it's ready to run, all of the domains are loaded and the server is ready to start.
